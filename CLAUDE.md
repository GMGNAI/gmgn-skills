...
| `gmgn-kline-pattern` | Names the chart pattern (uptrend channel / breakdown / basing / distribution …) and scores it 0-100 | User asks about 走势, 趋势, 形态, price action, chart pattern, "is it breaking down", "is it consolidating"; user wants a read of the chart rather than the raw candles |
| `gmgn-ca-verdict` | Contract safety + holder structure + price action combined into one 0-100 due-diligence score, every deduction with its reason | User pastes a contract address and asks 这个币怎么样, 安全吗, 能不能买, 尽调, "is this a rug", "audit this contract"; the default for an unqualified "看看这个币" |
| `gmgn-x-narrative` | X narrative & chatter — whether anyone is actually talking about the token, with explicit coverage reporting | User asks 有人聊吗, 叙事, 热度, 声量, KOL 提了吗, "is anyone talking about this", "social buzz"; **slow — up to a minute, do not run it routinely** |
...
| "看看这个币", "安全吗", "能不能买", contract address with no further qualifier | `gmgn-ca-verdict` |
| "走势怎么样", "什么形态", "is it breaking down" | `gmgn-kline-pattern` |
| "有人聊吗", "叙事", "热度", "social buzz" | `gmgn-x-narrative` |
...
- **`skills/`** — 11 skill definitions for Claude Code (SKILL.md, plus a Python analysis script where the skill computes something)
